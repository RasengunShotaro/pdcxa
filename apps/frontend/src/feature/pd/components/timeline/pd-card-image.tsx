...
export function PdCardImage({ imageFileName, alt }: PdCardImageProps) {
  const { data, isPending, error } = useS3Image(imageFileName);
  ...
  return (
    <div className="relative aspect-[3/2] w-full overflow-hidden rounded-lg border border-border bg-muted">
      ...
      {data && !hasError ? (
        // biome-ignore lint/performance/noImgElement: R2 から取得した ObjectURL を表示するため next/image は使えない
        <img
          alt={alt}
          className="absolute inset-0 size-full object-cover"
          onError={() => setFailed(true)}
          src={ArrayBufferToUrl(data)}
        />
      ) : null}
    </div>
  );
