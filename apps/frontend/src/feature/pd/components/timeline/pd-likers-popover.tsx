...
export function PdLikersPopover({
  ...
  if (likeCount === 0) {
    return (
      <span className="inline-flex h-9 items-center pr-2 pl-1 text-xs text-muted-foreground tabular-nums">
        0
      </span>
    );
  }
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          ...
          className="h-9 rounded-full pr-2.5 pl-1 text-xs text-muted-foreground tabular-nums hover:bg-transparent dark:hover:bg-transparent"
          type="button"
          variant="ghost"
        >
          {likeCount}
