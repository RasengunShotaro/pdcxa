...
| 「自作 harness が 9/9 pass したからルールは健全」      | 自作の評価環境は無意識に成功側へ寄る (self-report 指示で過剰熟考 / 要約しすぎたルールで対象が proportionally 目立つ / cold start で会話 momentum 欠如)。**依頼者の実観察 > 自作 harness の合成結果**。依頼者が「実際は失敗した」と言うなら依頼者を信じ、報告された機構を直接潰して実環境で再検証してもらう。 |
