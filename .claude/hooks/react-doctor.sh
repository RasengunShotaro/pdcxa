...
# react-doctor は frontend (React) 専用。config (frontend/doctor.config.json) は
# cwd 基準で解決されるため frontend を cwd にして起動する。
# 注意: この cd は react-doctor install が生成する既定スクリプト ($project_root) を手動調整したもの。
# `react-doctor install` を再実行するとこの行が上書きされるので、その場合は再度 /frontend を付け直す。
if ! cd "$project_root/apps/frontend"; then
  exit 0
fi
